...
func newTxSender(rest *restClient, privateKey string, accountIndex int64, apiKeyIndex uint8, chainID uint32, priceProtection bool) (*txSender, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("lighter: 初始化签名客户端失败: %w", err)
	}
	return &txSender{
		tx:              tx,
		rest:            rest,
...
func (s *txSender) currentNonce(ctx context.Context) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("lighter: 获取 nonce 失败: %w", err)
	}
	s.nonce = n
	s.hasNonce = true
	return s.nonce, nil
...
func (s *txSender) authToken() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("lighter: 生成 auth token 失败: %w", err)
	}
	s.cachedAuth = token
