...
// 端点地址。
const (
	mainnetREST = "https://api.rh.lighter.xyz"
	testnetREST = "https://api.rh-testnet.lighter.xyz"
	mainnetWS   = "wss://api.rh.lighter.xyz/stream"
	testnetWS   = "wss://api.rh-testnet.lighter.xyz/stream"

	mainnetChainID = 466324
	testnetChainID = 300
)
...
func (c *restClient) account(ctx context.Context, accountIndex int64) (*accountInfo, error) {
	...
	if len(out.Accounts) == 0 {
		return nil, fmt.Errorf("rh_lighter: 账户 %d 不存在", accountIndex)
	}
