...
// RH 主网公布费率（百分数）。orderBooks 若返回 0，自算已实现盈亏仍按此扣 maker 费。
var (
	rhMakerFeePercent = decimal.RequireFromString("0.012")
	rhTakerFeePercent = decimal.RequireFromString("0.035")
)

func feeRateFromPercent(pct, fallbackPct decimal.Decimal) decimal.Decimal {
	if !pct.IsPositive() {
		pct = fallbackPct
	}
...
func toMarket(d orderBookDetail) market.Market {
	...
	m := market.Market{
		...
		// 交易所把费率以百分数返回（"0.0120" 表示 0.012%），这里转成小数比率。
		// API 若给 0，用 RH 公布的 maker 0.012% / taker 0.035% 兜底，自算盈亏才扣得上费。
		MakerFeeRate:    feeRateFromPercent(d.MakerFee.Decimal, rhMakerFeePercent),
		TakerFeeRate:    feeRateFromPercent(d.TakerFee.Decimal, rhTakerFeePercent),
