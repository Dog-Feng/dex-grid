...
func TestToMarketConvertsFeeAndMarginUnits(t *testing.T) {
	...
	if !m.TakerFeeRate.Equal(dec("0.0005")) {
		t.Errorf("taker 费率 = %s，期望 0.0005（0.05%% 转成小数）", m.TakerFeeRate)
	}
...
func TestToMarketFallsBackToPublishedRHFees(t *testing.T) {
	d := orderBookDetail{
		orderBook: orderBook{
			Symbol:         "BTC",
			MarketID:       1,
	...
	m := toMarket(d)
	if !m.MakerFeeRate.Equal(dec("0.00012")) {
		t.Fatalf("maker fallback = %s, want 0.00012 (0.012%%)", m.MakerFeeRate)
	}
	if !m.TakerFeeRate.Equal(dec("0.00035")) {
		t.Fatalf("taker fallback = %s, want 0.00035 (0.035%%)", m.TakerFeeRate)
	}
}
