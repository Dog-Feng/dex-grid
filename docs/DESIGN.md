...
每闭合一个循环，毛利按**真实成交均价**配对（不是格子 High−Low）。页面「已实现」：交易所成交给出非 0 净盈亏时用该值（已扣费，不再二次扣）；字段为 0 或没有成交历史时用毛利 − 自估手续费（maker 成交按 maker 费率，止损/超时吃单按 taker 费率）。RH 的 `ask/bid_account_pnl` 常为 0，因此走自算。
