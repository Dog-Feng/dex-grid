...
每闭合一个循环，毛利按**真实成交均价**配对（不是格子 High−Low）。页面「已实现」：交易所成交给出非 0 净盈亏时用该值（DEX 已扣费，不再二次扣）；字段为 0 或没有成交历史时用闭合循环价差，**不再用估算费率二次扣减**（RH 的 `ask/bid_account_pnl` 常为 0，二次扣减会和交易历史对不上）。手续费仍记在 `fee_paid`。
