// Command rhlighterctl 是 RH Lighter 适配器的主网验证工具。
//
// 独立于 lighterctl：连的是 api.rh.lighter.xyz，用另一套账户与 chain_id=466324。
...
import (
	...
	_ "dex-grid/internal/exchange/lighter" // 只占用 ClientOrderID slot 0，保证 rh_lighter 为 slot 1

	"github.com/shopspring/decimal"
)

const usage = `rhlighterctl — RH Lighter 主网验证工具

本系统只做永续合约网格，所有命令的标的都是永续合约；
现货市场与已下架的合约不会出现在列表里，也不允许开仓。

用法:
  rhlighterctl [全局参数] <命令> [命令参数]
...
示例:
  ...
  rhlighterctl book -m 2
  rhlighterctl maker -m 2 -side buy -qty 0.15 -offset 0.02 -yes
  rhlighterctl cancel -m 2 -coid 17592186044417 -yes
`

func init() {
	// 空白导入 lighter 已占用 slot 0，这里追加 rh_lighter 为 slot 1，与 gridbot 一致。
...
func run() error {
	fs := flag.NewFlagSet("rhlighterctl", flag.ContinueOnError)
...
func newApp(cfgPath string) (*app, error) {
	...
	exCfg, ok := cfg.Find(rhlighter.Name)
	if !ok || !exCfg.Enabled {
		return nil, fmt.Errorf("配置里没有启用的 rh_lighter 交易所")
	}
	...
	ad, ok := ex.(*rhlighter.Adapter)
	if !ok {
		return nil, errors.New("内部错误：交易所适配器类型不是 rh_lighter")
	}
	return &app{
		cfg: cfg,
		ex:  ad,
...
func newCOID(cell int, purpose order.Purpose) (order.ClientOrderID, error) {
	slot, err := exchange.Slot(rhlighter.Name)
	if err != nil {
		return 0, err
	}
