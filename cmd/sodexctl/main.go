// Command sodexctl 是 SODEx 永续合约适配器的验证工具。
//
// 空白导入 lighter、注册 rh_lighter 只为占用 ClientOrderID slot 0 和 1，
// 保证本工具发出的测试单与 gridbot 一样落在 slot 2。它不会去连那两家 DEX。
...
import (
	...
	_ "dex-grid/internal/exchange/lighter" // slot 0

	"github.com/shopspring/decimal"
)

const usage = `sodexctl — SODEx 永续合约验证工具

本系统只做永续合约网格，所有命令的标的都是永续合约（BTC-USD 这类）；
现货市场与已暂停的合约不会出现在列表里，也不允许开仓。

用法:
  sodexctl [全局参数] <命令> [命令参数]
...
只读命令:
  ...
  check       校验凭证（主钱包地址 + accountID）

写入命令（必须加 -yes）:
  maker       挂一笔 post-only 限价单
  taker       下一笔市价单
  modify      改已存活挂单的价格和数量
  leverage    设置某个市场的杠杆与保证金模式
  cancel      按客户端订单号撤单
...
示例:
  ...
  sodexctl modify -m 1 -coid 8796093022209 -price 80000 -qty 0.001 -yes
  sodexctl cancel -m 1 -coid 8796093022209 -yes
`

func init() {
	exchange.Register(rhlighter.Name, rhlighter.New) // slot 1
	exchange.Register(sodex.Name, sodex.New)         // slot 2
}
...
func run() error {
	fs := flag.NewFlagSet("sodexctl", flag.ContinueOnError)
	...
	case "modify":
		return app.modify(ctx, rest)
	case "leverage":
		return app.leverage(ctx, rest)
	case "cancel":
...
func newApp(cfgPath string) (*app, error) {
	...
	exCfg, ok := cfg.Find(sodex.Name)
	if !ok || !exCfg.Enabled {
		return nil, fmt.Errorf("配置里没有启用的 sodex 交易所")
	}
	...
	ad, ok := ex.(*sodex.Adapter)
	if !ok {
		return nil, errors.New("内部错误：交易所适配器类型不是 sodex")
	}
	return &app{
		cfg: cfg,
		ex:  ad,
...
func (a *app) check(ctx context.Context) error {
	...
	fmt.Println("凭证校验通过：主钱包地址与 accountID 匹配，余额可查询")
	return nil
}

func (a *app) leverage(ctx context.Context, args []string) error {
	fs := flag.NewFlagSet("leverage", flag.ContinueOnError)
	idx := fs.Int("m", -1, "market_index")
	x := fs.Int("x", 0, "杠杆倍数")
	mode := fs.String("mode", "isolated", "cross | isolated")
	yes := fs.Bool("yes", false, "确认执行")
	if err := fs.Parse(args); err != nil {
		return err
	}
	if *idx < 0 || *x < 1 {
		return errors.New("需要 -m 与 -x（杠杆倍数 ≥ 1）")
	}
	mm, err := market.ParseMarginMode(*mode)
	if err != nil {
		return err
	}
	symbol, _, err := a.resolveSymbol(ctx, *idx)
	...
	fmt.Printf("设置 %s 杠杆 %dx，保证金模式 %s\n", symbol, *x, mm)
	if !*yes {
		fmt.Println("演练模式，未发送。确认无误后加 -yes 执行。")
		return nil
	}
	if err := a.ex.SetLeverage(ctx, symbol, *x, mm); err != nil {
		return err
	}
	fmt.Println("已提交")
	return nil
...
func newCOID(cell int, purpose order.Purpose) (order.ClientOrderID, error) {
	slot, err := exchange.Slot(sodex.Name)
	if err != nil {
		return 0, err
	}
...
func (a *app) submit(ctx context.Context, req exchange.PlaceRequest) error {
	...
	fmt.Printf("已提交，order_id=%s\n", id)
	return nil
}

func (a *app) modify(ctx context.Context, args []string) error {
	fs := flag.NewFlagSet("modify", flag.ContinueOnError)
	idx := fs.Int("m", -1, "symbolID（markets 列表的 INDEX）")
	coid := fs.Int64("coid", 0, "客户端订单号")
	price := fs.String("price", "", "新价格")
	qtyStr := fs.String("qty", "", "新数量")
	yes := fs.Bool("yes", false, "确认执行")
	if err := fs.Parse(args); err != nil {
		return err
	}
	if *idx < 0 || *coid == 0 || *price == "" || *qtyStr == "" {
		return errors.New("需要 -m、-coid、-price、-qty")
	}
	symbol, m, err := a.resolveSymbol(ctx, *idx)
	if err != nil {
		return err
	...
	if err != nil {
		return fmt.Errorf("-qty 不是合法数字: %w", err)
	}
	p = m.RoundPrice(p, market.RoundNearest)
	q = m.RoundQty(q)
	if err := m.CheckOrder(p, q); err != nil {
		return fmt.Errorf("不满足市场限制: %w", err)
	}
	fmt.Printf("改单 %s coid=%d → 价格 %s 数量 %s\n", symbol, *coid, p, q)
	if !*yes {
		fmt.Println("演练模式，未发送。确认无误后加 -yes 执行。")
		return nil
	}
	results, err := a.ex.ModifyOrders(ctx, []exchange.ModifyRequest{{
		Symbol:        symbol,
...
func (a *app) cancel(ctx context.Context, args []string) error {
	...
	idx := fs.Int("m", -1, "market_index")
	coid := fs.Int64("coid", 0, "客户端订单号")
	oid := fs.String("oid", "", "交易所订单号（没有客户端订单号时用）")
	yes := fs.Bool("yes", false, "确认执行")
	if err := fs.Parse(args); err != nil {
		return err
	}
	if *idx < 0 || (*coid == 0 && *oid == "") {
		return errors.New("需要 -m，以及 -coid 或 -oid")
	}
	symbol, _, err := a.resolveSymbol(ctx, *idx)
	if err != nil {
		return err
